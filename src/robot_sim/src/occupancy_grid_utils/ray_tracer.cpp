...
inline bool cellWithinBounds(const nm::MapMetaData& info, const Cell& c)
{
...
optional<Cell> rayTraceOntoGrid(const nm::MapMetaData& info, const Cell& c1, const Cell& c2)
{
  ...
  RayTraceIterator pos =
      std::find_if(r.first, r.second, bind(cellWithinBounds, boost::ref(info), _1));
  optional<Cell> c;
  if (pos != r.second)
    c = *pos;
  return c;
                           ...
                           float max_range)
{
  ...
  ROS_DEBUG_STREAM_NAMED("ray_trace", "Ray tracing between " << c1.x << ", " << c1.y << " and "
                                                             << c2.x << ", " << c2.y);
  ...
  ROS_DEBUG_STREAM_NAMED("ray_trace", "Projected ray trace endpoints to " << c1.x << ", " << c1.y
                                                                          << " and " << c2.x << ", "
                                                                          << c2.y);
...
{
private:
  ScanEndPoints() : angle_min_(0.0), angle_max_(0.0), range_max_(0.0), angle_increment_(0.0) {}
...
sm::LaserScan::Ptr simulateRangeScan(const nm::OccupancyGrid& grid, const gm::Pose& sensor_pose,
