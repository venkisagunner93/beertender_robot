...
#include <robot_sim/LocalizedCloud.h>
#include <robot_sim/OverlayClouds.h>
...
/// are marked UNKNOWN
robot_sim::OverlayClouds createCloudOverlay (const nav_msgs::OccupancyGrid& grid, const std::string& frame_id,
...
void addCloud (robot_sim::OverlayClouds* overlay, robot_sim::LocalizedCloud::ConstPtr cloud);
...
void removeCloud (robot_sim::OverlayClouds* overlay, robot_sim::LocalizedCloud::ConstPtr cloud);

/// \brief Assert that a square centered at this point with side 2*r contains no obstacles
void addKnownFreePoint (robot_sim::OverlayClouds* overlay, const geometry_msgs::Point& p,
                        double r);


/// \brief Get the current grid.  It's fine to modify the returned object.
nav_msgs::OccupancyGrid::Ptr getGrid (const robot_sim::OverlayClouds& overlay);

// \brief Reset all counts to initial state in \a overlay
void resetCounts (robot_sim::OverlayClouds* overlay);

// \brief Saturate hit counts to the closest value to the threshold
void saturateCounts (robot_sim::OverlayClouds& overlay);

// \brief Saturate hit counts to the closest (by a given number of steps/observation) value to the threshold
void saturateCounts (robot_sim::OverlayClouds& overlay, const size_t steps);

// \brief Get the occupancy grid dimensions.
nav_msgs::MapMetaData gridInfo (const robot_sim::OverlayClouds& overlay);
